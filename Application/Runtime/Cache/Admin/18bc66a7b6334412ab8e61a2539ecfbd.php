							...
							<table id="sample-table-1" class="table table-striped table-bordered table-hover">
								...
								<!-- 遍历表内容并输出 -->
									<?php if(is_array($orders)): foreach($orders as $key=>$order): ?><tr>
										...
										<!-- 根据订单状态不同显示不同信息 -->
										<?php switch($order['orders_status']): case "0": ?><td>新订单|
												<a class="send">发货</a></td><?php break;?>
											<?php case "1": ?><td>已发货</td><?php break;?>
											<?php case "2": ?><td>买方已收货</td><?php break;?>
											<?php case "3": ?><td>无效的订单</td><?php break;?>
											<?php default: ?>默认情况<?php endswitch;?>
										<!-- <td><?php echo ($order['orders_status']); ?></td> -->
										<td><a href="/shop/admin.php/Detail/index/orderid/<?php echo ($order["orders_id"]); ?>">查看详情</a></td>
									</tr><?php endforeach; endif; ?>

									<script type="text/javascript">
										$(".send").click(function(){
											var id = $(this).parent().prev().html();
											$.get("/shop/admin.php/Order/modify",{id:id});
											alert("成功发货");
