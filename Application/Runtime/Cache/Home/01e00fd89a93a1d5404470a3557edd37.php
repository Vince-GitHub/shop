...
<div id="member_left" class="flt">
    <div class="member_leftMenu">
        ...
        <ul>
           ...
           <li><a id="ChangePwd" href="/hszz/TP/index.php/MyOrder/repass">修改密码</a></li>
        </ul>
        ...
        <div class="member_content" id="spTabs_content">
                <!-- 暂无数据 Start -->
                 <table class="prodList" border="0" cellpadding="0" cellspacing="0">
                            ...
                            <tbody>
                            <!-- 遍历表内容并输出 -->
									<?php if(is_array($orders)): foreach($orders as $key=>$order): ?><tr style="border-bottom:1px solid #ccc">
										<td><?php echo ($order['orders_num']); ?></td>
										<td><?php echo ($order['orders_linkman']); ?></td>
										<td><?php echo ($order['orders_address']); ?></td>
										<td><?php echo ($order['orders_phone']); ?></td>
										<td><?php echo (date("Y-m-d",$order['orders_addtime'])); ?></td>
										<td><?php echo ($order['orders_total']); ?></td>
										<td style="display:none"><?php echo ($order['orders_id']); ?></td>
										<!-- 根据订单状态不同显示不同信息 -->
										<?php switch($order['orders_status']): case "0": ?><td>新订单</td><?php break;?>
											<?php case "1": ?><td>待收货|
											<a class="accept">确认收货</a></td><?php break;?>
											<?php case "2": ?><td>已收货</td><?php break;?>
											<?php case "3": ?><td>无效的订单</td><?php break;?>
											<?php default: ?>默认情况<?php endswitch;?>
										<!-- <td><?php echo ($order['orders_status']); ?></td> -->
										<td><a href="/hszz/TP/index.php/Detail/index/orderid/<?php echo ($order["orders_id"]); ?>">查看详情</a></td>
                        ...
                        <script type="text/javascript">
                        	$(".accept").click(function(){
                        		...
                        		$.get("/hszz/TP/index.php/MyOrder/modify",{id:id});
								$(this).parent().html("已收货");
                        	});
                        </script>
