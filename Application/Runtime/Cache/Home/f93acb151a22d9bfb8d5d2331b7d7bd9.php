...
<div id="member_left" class="flt">
    <div class="member_leftMenu">
        ...
        <ul>
           ...
           <li><a id="ChangePwd" href="/hszz/TP/index.php/Share/repass">修改密码</a></li>
        </ul>
...
<div id="member_right" class="flt">
    ...
    <h2>
        我的分享<a href="/hszz/TP/index.php/List/index" class="mobiapp">+添加分享,请到商品详情页</a>
    </h2>
        ...
        <div class="member_content" id="spTabs_content">
                <!-- 暂无数据 Start -->
                 <table class="prodList" border="0" cellpadding="0" cellspacing="0">
                            ...
                            <tbody>
                            <!-- 遍历表内容并输出 -->
                                <?php if(is_array($shares)): foreach($shares as $key=>$share): ?><tr style="border-bottom:1px solid #ccc">
                                    <!--评论标题-->
                                    <td><?php echo ($share['share_title']); ?></td>
                                    <!--评论内容-->
									<td><?php echo ($share['share_content']); ?></td>

									<!--根据商品ID查看商品名称-->
									<td>
										<?php if(is_array($goods)): foreach($goods as $key=>$good): if($good['goods_id'] == $share['share_goodsid']): echo ($good['goods_goods']); endif; endforeach; endif; ?>
									</td>
									
									<!--根据商品ID查看商品图片-->
									<td>
										<?php if(is_array($goods)): foreach($goods as $key=>$good): if($good['goods_id'] == $share['share_goodsid']): ?><img width=60px height=60px src="/hszz/TP/Public/uploads/<?php echo ($good['goods_picname']); ?>"/><?php endif; endforeach; endif; ?>
									</td>
									
									<!--分享时间-->
                                    <td><?php echo (date("Y-m-d H:i",$share['share_time'])); ?></td>
                                    <td><a href="/hszz/TP/index.php/share/del/id/<?php echo ($share['share_id']); ?>">删除</a></td>
