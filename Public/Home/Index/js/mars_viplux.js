! function() {
	window.T0 = (new Date).getTime(), window.Mar = function() {}, Mar.Cookie = {
		...
		var c = Mar.Base,
			d = Mar.Biz();
		a = Mar.protocal() + "mar.vip.com" + a + "&mars_cid=" + d.cid + "&mars_sid=" + d.sid + "&pi=" + d.pid + "&mars_vid=" + d.vid + "&lvm_id=" + d.lvmId + "&mars_var=" + d.mvar + "&lg=" + d.isLog + "&wh=" + d.wh + "&in=" + d.newbie + "&sn=" + d.orderId + "&url=" + c.url + "&sr=" + c.res + "&rf=" + c.ref + "&bw=" + c.cw + "&bh=" + c.ch + "&sc=" + c.col + "&bv=" + c.nav + "&ce=" + c.ce + "&vs=&title=" + c.title + "&tab_page_id=" + d.pageId, b && (a += "&wap_ln=" + d.wapln + "&wap_vs=" + d.wapvs + "&wap_pwh=" + d.wappwh + "&wap_wh=" + d.wapwh + "&wap_id=" + d.wapid + "&wap_from=" + d.wapfrom + "&cps_u=" + d.cpsu + "&m_vipruid=" + d.ruid), a += "&r=" + Math.random();
		var e = new Image(1, 1);
...
function(a) {
	Mar.PageIdIndex = 0, Mar.PageId = Mar.PageId ? Mar.PageId + "_" + ++Mar.PageIdIndex : (new Date).getTime() + "_" + Mar.guid2(), Mar.Biz = function(b) {
		...
		return Mar.Request("/viplux/aa?"), Mar
	}
}(jQuery),
function(a) {
	Mar.Seed = function() {
		...
		function c(c, e) {
			...
			e = e || "click", "a" === j ? (g = i.attr("href"), h = d[j + (g ? b(g.toLowerCase()) : "")]) : (g = i.attr("type"), f = d[j + (g ? g.toLowerCase() : "")], h = f ? f : j), Mar.Seed.request(h, e, i.attr("hover" !== e.toLowerCase() ? "mars_sead" : "mars_sead_hover"), i.attr("data-mars"))
		}
		var d = {
			inputbutton: "button",
			inputsubmit: "button",
		...
		d = d || "", Mar.Request("/viplux/ab?at=" + e + "&et=" + a + "&ed=" + b + "&one=" + encodeURIComponent(c) + "&data_mars=" + d)
	}
}(jQuery),
function() {
	Mar.PV().Seed()
